...
for _ in $(seq 1 60); do
  if curl --fail --silent --show-error --max-time 10 "$base_url/api/backend/health/ready" >"$tmp_dir/ready.json"; then
    break
  fi
  sleep 2
done
...
curl --fail --silent --show-error --max-time 10 -D "$tmp_dir/headers" "$base_url/api/backend/catalog/products" >"$tmp_dir/catalog.json"
grep -qi '^x-request-id:' "$tmp_dir/headers"
jq -e '.items | length >= 2' "$tmp_dir/catalog.json" >/dev/null

buyer_token=$(curl --fail --silent --show-error --max-time 10 "$base_url/api/backend/auth/demo-login" \
  -H 'Content-Type: application/json' -d '{"role":"buyer"}' | jq -r .accessToken)
[[ -n "$buyer_token" && "$buyer_token" != null ]]
curl --fail --silent --show-error --max-time 10 "$base_url/api/backend/cart" \
