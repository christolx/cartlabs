...
catalog_query="$api_url/catalog/products?q=basket&store=nusantara-goods&page=1&pageSize=20"
baseline=$(curl --fail --silent --show-error "$catalog_query" | jq -c '{total,names:[.items[].name],stores:[.items[].storeSlug]}')
...
fallback=$(curl --fail --silent --show-error "$catalog_query" | jq -c '{total,names:[.items[].name],stores:[.items[].storeSlug]}')
...
recovered=$(curl --fail --silent --show-error "$catalog_query" | jq -c '{total,names:[.items[].name],stores:[.items[].storeSlug]}')
