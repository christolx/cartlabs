...
function StoreContent() {
  ...
  const load = useCallback(async () => {
    setError("");
    try {
      setStore(await request<Store>("/seller/store"));
    } catch (cause) {
      if (cause instanceof BrowserAPIError && cause.status === 404)
        setStore(null);
      else setError(errorMessage(cause, "Store unavailable."));
    }
  }, [request]);
  useEffect(() => {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const next = await request<Store>("/seller/store", {
      ...
      setMessage(
        store
          ? "Store saved."
          : "Store created and submitted for verification.",
      );
    } catch (cause) {
      setMessage(errorMessage(cause, "Store save failed."));
    } finally {
      setBusy(false);
    }
  }
    ...
    <>
      ...
      {store?.status === "rejected" ? (
        <div className="notice notice-danger">
          ...
          <p>{store.moderationNote || "No verification note provided."}</p>
        </div>
