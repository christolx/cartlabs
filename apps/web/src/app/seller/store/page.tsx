...
function StoreContent() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStore(next); setMessage(store ? "Store saved. Material edits return to pending review." : "Store created and submitted for review.");
  ...
  return <><PageHeading title={store ? "Manage store" : "Create store"} description="Approved store status is required before product supply can be created." actions={store ? <Status value={store.status} /> : undefined} />{store?.status === "rejected" ? <div className="notice notice-danger"><strong>Store rejected</strong><p>{store.moderationNote || "No moderation note provided."}</p></div> : store?.status === "pending" ? <div className="notice"><strong>Review pending</strong><p>Products cannot be created until admin approves this store.</p></div> : null}<section className="form-panel"><form className="stack-form" onSubmit={submit}><label><span>Store name</span><input name="name" minLength={2} maxLength={120} defaultValue={store?.name} required disabled={busy} /></label><label><span>Store slug</span><input name="slug" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" maxLength={80} defaultValue={store?.slug} required disabled={busy} /></label><label><span>Description</span><textarea name="description" minLength={2} maxLength={2000} rows={6} defaultValue={store?.description} required disabled={busy} /></label>{store ? <p className="helper-text">Changing store identity sends approved store back to pending moderation.</p> : null}{message ? <p className="form-message" role="status">{message}</p> : null}<button className="button button-primary" disabled={busy}>{busy ? "Saving" : store ? "Save store" : "Create store"}</button></form></section></>;
}
