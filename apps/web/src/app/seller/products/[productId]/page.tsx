...
function ProductManagementContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(errorMessage(cause, "Product unavailable or not owned by this seller."));
    }
  }, [productId, request]);
  ...
  async function addVariant(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const parsed = JSON.parse(String(data.get("attributes") || "{}")) as unknown;
  ...
  return <>
    <PageHeading title={product.name} description="Owned product detail, supply, inventory, images, and publication." actions={<Status value={product.status} />} />
    {product.status === "suspended" ? <div className="notice notice-danger"><strong>Listing suspended</strong><p>You may update content and inventory, but only admin can reinstate publication.</p></div> : null}
    ...
    <div className="management-grid">
      <section className="form-panel">
        ...
        <form className="stack-form" onSubmit={updateProduct}>
          <label><span>Category</span><select name="categoryId" defaultValue={product.category.id} required disabled={Boolean(busy)}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
          <label><span>Name</span><input name="name" defaultValue={product.name} required disabled={Boolean(busy)} /></label>
          <label><span>Slug</span><input name="slug" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" defaultValue={product.slug} required disabled={Boolean(busy)} /></label>
          <label><span>Description</span><textarea name="description" rows={6} defaultValue={product.description} required disabled={Boolean(busy)} /></label>
          <button className="button button-primary" disabled={Boolean(busy)}>Save identity</button>
        </form>
      </section>
      <section className="form-panel">
        <div className="panel-heading-row"><div><h2>Variants and inventory</h2><p>Existing variants cannot be edited or deleted.</p></div><span className="count-badge">{product.variants.length}</span></div>
        <div className="variant-management-list">{product.variants.map((variant) => <article key={variant.id}>
          <div className="heading-status"><div><h3>{variant.name}</h3><span>{variant.sku}</span></div><strong><Money value={variant.priceMinor} /></strong></div>
          <p>{variant.stock} in stock{Object.keys(variant.attributes).length ? `. ${Object.entries(variant.attributes).map(([key, value]) => `${key}: ${value}`).join(", ")}` : ""}</p>
        ...
        <details className="management-disclosure"><summary>Add variant</summary><form className="stack-form two-column-form compact-form" onSubmit={addVariant}><label><span>SKU</span><input name="sku" required /></label><label><span>Variant name</span><input name="name" required /></label><label><span>Price (IDR)</span><input name="priceMinor" type="number" min="0" required /></label><label><span>Initial stock</span><input name="stock" type="number" min="0" required /></label><label className="form-wide"><span>Attributes JSON</span><input name="attributes" defaultValue="{}" /></label><button className="button button-primary form-wide" disabled={Boolean(busy)}>Add variant</button></form></details>
      </section>
      <ProductImageManager productId={productId} status={product.status} images={product.images} onChanged={load} />
      <section className="form-panel submit-panel">
        <h2>Listing status</h2>
        {product.status === "published" ? <><p>Archive removes listing from marketplace until you republish it.</p><button className="button button-secondary" type="button" disabled={Boolean(busy)} onClick={() => void run("archive", `/seller/products/${productId}/archive`, { method: "POST" })}>{busy === "archive" ? "Archiving" : "Archive listing"}</button></> : product.status === "suspended" ? <p>Admin reinstatement required.</p> : <><p>Publishing is immediate. Product needs active in-stock variant and image.</p><button className="button button-primary" type="button" disabled={!canPublish || Boolean(busy)} onClick={() => void run("publish", `/seller/products/${productId}/publish`, { method: "POST" })}>{busy === "publish" ? "Publishing" : product.status === "archived" ? "Republish" : "Publish"}</button>{!canPublish ? <p className="helper-text">Add active stock and image first.</p> : null}</>}
      </section>
    </div>
  </>;
}
