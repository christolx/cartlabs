...
function ProductManagementContent() {
  ...
  const canEdit = product.status === "draft" || product.moderationStatus === "rejected";
  const canSubmit = product.status === "draft" && product.variants.length > 0 && product.images.length > 0;
  return <><PageHeading title={product.name} description="Owned product detail, supply, inventory, images, and moderation." actions={<div className="status-pair"><Status value={product.status} /><Status value={product.moderationStatus} /></div>} />{product.moderationStatus === "rejected" ? <div className="notice notice-danger"><strong>Product rejected</strong><p>{product.moderationNote || "No moderation note provided."}</p></div> : null}{message ? <p className="action-message" role="status">{message}</p> : null}<div className="management-grid"><section className="form-panel"><h2>Product identity</h2><form className="stack-form" onSubmit={updateProduct}><label><span>Category</span><select name="categoryId" defaultValue={product.category.id} required disabled={!canEdit || Boolean(busy)}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label><span>Name</span><input name="name" defaultValue={product.name} required disabled={!canEdit || Boolean(busy)} /></label><label><span>Slug</span><input name="slug" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" defaultValue={product.slug} required disabled={!canEdit || Boolean(busy)} /></label><label><span>Description</span><textarea name="description" rows={6} defaultValue={product.description} required disabled={!canEdit || Boolean(busy)} /></label><button className="button button-primary" disabled={!canEdit || Boolean(busy)}>Save identity</button></form></section><section className="form-panel"><div className="panel-heading-row"><div><h2>Variants and inventory</h2><p>Existing variants cannot be edited or deleted.</p></div><span className="count-badge">{product.variants.length}</span></div><div className="variant-management-list">{product.variants.map((variant) => <article key={variant.id}><div className="heading-status"><div><h3>{variant.name}</h3><span>{variant.sku}</span></div><strong><Money value={variant.priceMinor} /></strong></div><p>{variant.stock} in stock{Object.keys(variant.attributes).length ? `. ${Object.entries(variant.attributes).map(([key, value]) => `${key}: ${value}`).join(", ")}` : ""}</p><details><summary>Adjust inventory</summary><form className="stack-form compact-form" onSubmit={(event) => void adjustInventory(event, variant.id)}><label><span>Change</span><input name="delta" type="number" required min={-1000000} max={1000000} /></label><label><span>Reason</span><input name="reason" minLength={2} maxLength={500} required /></label><button className="button button-secondary" disabled={Boolean(busy)}>Apply adjustment</button></form></details></article>)}</div>{canEdit ? <details className="management-disclosure"><summary>Add variant</summary><form className="stack-form two-column-form compact-form" onSubmit={addVariant}><label><span>SKU</span><input name="sku" required /></label><label><span>Variant name</span><input name="name" required /></label><label><span>Price (IDR)</span><input name="priceMinor" type="number" min="0" required /></label><label><span>Initial stock</span><input name="stock" type="number" min="0" required /></label><label className="form-wide"><span>Attributes JSON</span><input name="attributes" defaultValue="{}" /></label><button className="button button-primary form-wide" disabled={Boolean(busy)}>Add variant</button></form></details> : null}</section><section className="form-panel"><div className="panel-heading-row"><div><h2>Image URLs</h2><p>Register hosted image metadata. Binary upload is unsupported.</p></div><span className="count-badge">{product.images.length}</span></div>{product.images.length ? <div className="image-management-grid">{product.images.map((image) => <figure key={image.id}><Image src={image.url} alt={image.altText} width={240} height={180} /><figcaption>{image.altText || "No alt text"} / position {image.position}</figcaption></figure>)}</div> : <p className="empty-copy">No images registered.</p>}{canEdit ? <details className="management-disclosure"><summary>Add image URL</summary><form className="stack-form compact-form" onSubmit={addImage}><label><span>Image URL</span><input name="url" required /></label><label><span>Alt text</span><input name="altText" maxLength={240} required /></label><label><span>Position</span><input name="position" type="number" min="0" defaultValue="0" required /></label><button className="button button-primary" disabled={Boolean(busy)}>Add image</button></form></details> : null}</section><section className="form-panel submit-panel"><h2>Moderation submission</h2><p>Draft needs at least one variant and one image. Approved product becomes public catalog supply.</p><button className="button button-primary" type="button" disabled={!canSubmit || Boolean(busy)} onClick={() => void run("publish", `/seller/products/${productId}/publish`, { method: "POST" })}>{busy === "publish" ? "Submitting" : "Submit for review"}</button>{!canSubmit && product.status === "draft" ? <p className="helper-text">Add missing variant or image first.</p> : null}</section></div></>;
}
