...
function NewProductContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setCategories(
        (await request<{ items: Category[] }>("/categories")).items,
      );
    } catch (cause) {
      setError(errorMessage(cause, "Categories unavailable."));
    }
  }, [request]);
  useEffect(() => {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const product = await request<Product>("/seller/products", {
        method: "POST",
        body: JSON.stringify({
      ...
      setMessage(
        errorMessage(
          cause,
          "Product creation failed. Verified store required.",
        ),
      );
    } finally {
      setBusy(false);
