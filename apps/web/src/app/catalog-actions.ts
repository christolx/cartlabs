...
export async function loadCatalogPage(filterQuery: string, excludeIds: string[]) {
  if (!Array.isArray(excludeIds) || excludeIds.length > 1_000 || excludeIds.some((id) => !/^[0-9a-f-]{36}$/.test(id))) {
  ...
  if (category) {
    if (category.length > 100 || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(category)) throw new Error("Invalid category");
    query.set("category", category);
  }
  if (minPrice !== undefined) query.set("minPrice", String(minPrice));
  if (maxPrice !== undefined) query.set("maxPrice", String(maxPrice));
  if (minPrice !== undefined && maxPrice !== undefined && minPrice > maxPrice) throw new Error("Invalid price range");
  ...
  do {
    ...
    const result = await apiGet<ProductPage>(`/catalog/products?${query.toString()}`);
  ...
  return { items: items.slice(0, 8), hasMore: items.length > 8 || hasAnotherPage };
}
