...
async function proxy(request: Request, context: RouteContext<"/api/backend/[...path]">) {
  ...
  for (const name of ["accept", "authorization", "content-type", "cookie", "idempotency-key", "x-request-id"]) {
    const value = request.headers.get(name);
    if (value) headers.set(name, value);
  }
  const contentLength = Number(request.headers.get("content-length") ?? 0);
  if (contentLength > 1_048_576) return Response.json({ detail: "Request body too large" }, { status: 413 });
  const body = request.method === "GET" || request.method === "HEAD" ? undefined : await request.arrayBuffer();
  if (body && body.byteLength > 1_048_576) return Response.json({ detail: "Request body too large" }, { status: 413 });
  try {
