...
async function uploadToCloudinary(file: File, onProgress: (value: number) => void) {
  ...
  if (!config.cloudName || !config.uploadPreset) throw new Error("Cloudinary upload is not configured.");
    ...
    request.addEventListener("load", () => {
      try {
        const response = JSON.parse(request.responseText) as { secure_url?: string; error?: { message?: string } };
...
export function ProductImageManager({ productId, status, images, onChanged }: {
  productId: string;
  status: string;
  images: ProductImage[];
  ...
  async function addImage(event: FormEvent<HTMLFormElement>) {
    ...
    if (!file) { setMessage("Choose image first."); return; }
    setBusy("add"); setMessage(""); setProgress(0);
    try {
      await upload(file, `/seller/products/${productId}/images`, "POST", altText);
      setFile(null); setAltText(""); setPreview("");
      if (inputRef.current) inputRef.current.value = "";
      await onChanged(); setMessage("Image added.");
    } catch (cause) { setMessage(errorMessage(cause, "Image upload failed.")); }
    finally { setBusy(""); setProgress(0); }
  }

  async function replaceImage(event: ChangeEvent<HTMLInputElement>, image: ProductImage) {
  ...
  async function deleteImage(imageId: string) {
    if (!window.confirm("Delete this product image? Remaining images will shift forward.")) return;
    setBusy(imageId); setMessage("");
    try {
      await request(`/seller/products/${productId}/images/${imageId}`, { method: "DELETE" });
      await onChanged(); setMessage("Image deleted.");
    } catch (cause) { setMessage(errorMessage(cause, "Image deletion failed.")); }
    finally { setBusy(""); }
  }

  const atLimit = images.length >= 8;
  const lastPublishedImage = status === "published" && images.length === 1;
  return <section className="form-panel image-manager">
    <div className="panel-heading-row"><div><h2>Product images</h2><p>Upload JPEG, PNG, WebP, or AVIF. 5 MB and 4096 px maximum.</p></div><span className="count-badge">{images.length} / 8</span></div>
    {images.length ? <div className="image-management-grid">{images.map((image) => <figure key={image.id}>
      <Image src={image.url} alt={image.altText} width={240} height={180} />
      <figcaption>{image.position === 0 ? "Cover · " : ""}{image.altText || "No alt text"}</figcaption>
      <label><span>Alt text for replacement</span><input value={replacementAlt[image.id] ?? image.altText} maxLength={160} onChange={(event) => setReplacementAlt((values) => ({ ...values, [image.id]: event.currentTarget.value }))} disabled={Boolean(busy)} /></label>
      <div className="image-actions">
        <label className="button button-secondary">{busy === image.id ? "Working" : "Replace"}<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" disabled={Boolean(busy)} onChange={(event) => void replaceImage(event, image)} /></label>
        <button className="text-button danger-button" type="button" disabled={Boolean(busy) || lastPublishedImage} title={lastPublishedImage ? "Published product must keep one image" : undefined} onClick={() => void deleteImage(image.id)}>Delete</button>
      </div>
