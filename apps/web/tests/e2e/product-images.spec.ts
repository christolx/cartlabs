...
async function mockProductPage(
  ...
  await page.route("**/api/cloudinary-config", (route) =>
    json(route, { cloudName: "demo-cloud", uploadPreset: "cartlabs-upload" }),
  );
  await page.route("https://api.cloudinary.com/**", (route) =>
    json(route, {
      secure_url:
        "https://res.cloudinary.com/demo-cloud/image/upload/v1/lamp.webp",
    }),
  );
  await page.route("**/api/backend/**", async (route) => {
    ...
    if (url.pathname.endsWith("/categories"))
      return json(route, { items: [product().category] });
    if (
      url.pathname.endsWith(
        "/seller/products/11111111-1111-4111-8111-111111111111",
      ) &&
      route.request().method() === "GET"
    )
      return json(route, product(images, initialStatus));
    if (
      ...
      images = [
        {
          ...
          url: "https://res.cloudinary.com/demo-cloud/image/upload/v1/lamp.webp",
          altText: "Desk lamp",
          position: 0,
        },
      ];
    ...
    if (
      ...
      images = [
        {
          ...images[0],
          url: "https://res.cloudinary.com/demo-cloud/image/upload/v2/lamp.webp",
          altText: "Replacement lamp",
        },
      ];
...
test("seller uploads, replaces, and deletes product image", async ({
  page,
}) => {
...
test("seller cannot add ninth image or delete only published image", async ({
  ...
  const eightImages = Array.from({ length: 8 }, (_, position) => ({
    ...image,
    id: `55555555-5555-4555-8555-55555555555${position}`,
    position,
  }));
  ...
  await expect(
    page.getByRole("button", { name: "Eight-image limit reached" }),
  ).toBeDisabled();
  await expect(
    page.getByRole("button", { name: "Product image file" }),
  ).toBeDisabled();
});
